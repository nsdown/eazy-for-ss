...
def generate_ocserv(metric):
    results = fetch_ip_data()  
    rfile=open('routes.conf','w')
    for ip,mask,_ in results:
        route_item="no-route = %s/%s\n"%(ip,mask)
        rfile.write(route_item)
    rfile.close()
    print "Usage: Append the content of the newly created routes.conf to your ocserv(>=0.9.2) config file," \
          " and the maximum number of routing table rules at least %d ." % (len(results)+20)
